...
bp_ls.route('/end', methods=["PATCH"])(end_ls)
bp_ls.route('/check', methods=["GET"])(check_ls)

bp_watch.route('/start', methods=["POST"])(start_watching)
bp_watch.route('/stop', methods=["POST"])(stop_watching)
bp_comment.route('', methods=["POST"])(post_comment)
bp_comment.route('', methods=["GET"])(fetch_comments)
